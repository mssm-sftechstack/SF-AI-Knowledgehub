...
name: sf-security
description: >
  Salesforce security review skill. Run before every deployment. Covers SOQL injection,
  XSS, FLS violations, CRUD violations, sharing model gaps, hardcoded credentials,
  insecure callouts, open redirect, debug log exposure, insecure deserialization,
  LIMIT enforcement, Guest/Community user access risks, Shield Platform Encryption,
  Transaction Security Policies, and OWASP API Security Top 10 for Salesforce.
  Based on Salesforce Security Guide and OWASP Top 10 for Salesforce (API 62.0).
---
...
// CORRECT - dynamic record type lookup
...
Flag: 15/18-char Salesforce IDs as string literals; variables named `password`, `secret`, `apiKey`, `token` with hardcoded values.
