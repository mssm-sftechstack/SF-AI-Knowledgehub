...
name: sf-apex
description: >
  Salesforce Apex development skill. Covers Apex classes, trigger handlers, service layers,
  selector classes, batch jobs, queueable jobs, and scheduled jobs. Enforces bulkification,
  governor limits, security (FLS/CRUD/sharing), inherited sharing, and multi-tenant best
  practices. Based on Salesforce Apex Developer Guide (API 62.0).
allowed-tools: Bash(sf apex run*), Bash(sf project deploy*)
---
