...
Salesforce automatically indexes these fields. No setup required.
...
Custom fields NOT marked External ID or Unique are NOT indexed. Filtering on them alone forces a full table scan on large objects. To request a custom index, open a Salesforce Support case.
...
The child object name in the subquery is the **relationship name** (plural), not the API name.
...
List<Contact> contacts = [
    SELECT Id, FirstName, LastName,
           Account.Name, Account.BillingCity, Account.OwnerId
    FROM Contact
...
| Relationship traversal for dot notation | 5 levels |
...
Integer total = [SELECT COUNT() FROM Opportunity WHERE IsClosed = false];
...
Use static SOQL whenever possible -- it's compiled at deploy time. Use `Database.query()` only when field or object names are not known until runtime.
...
Introduced in API 56.0. Enforces FLS and CRUD at the database level for both reads and writes. Preferred for new service class development.

```apex
// USER_MODE on SELECT - throws QueryException if user lacks field or object read
List<Account> accs = [
...
Use `WITH USER_MODE` for all new service layer queries where possible.
...
| Rows per query (with LIMIT) | Up to 50,000 |
...
## 9. Checklist -- Before Any Query Ships

- [ ] Filters on at least one indexed field (Id, OwnerId, CreatedDate, ExternalId, Lookup)
- [ ] Result set is selective (fewer than ~10% of total records on large objects)
- [ ] No leading wildcard in any LIKE clause
- [ ] LIMIT present (or intentional bulk query with documented reason)
- [ ] `WITH SECURITY_ENFORCED` or `WITH USER_MODE` present (or `Security.stripInaccessible()` post-query)
- [ ] Query is outside all loops
- [ ] Bound variables used (`:varName`) -- no string concatenation of user input
- [ ] `SELECT *` / `FIELDS(ALL)` is NOT used
- [ ] Result assigned to a `List<>` -- never directly to a single SObject without isEmpty() guard
...
- [Apex SOQL Inline Queries](https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/langCon_apex_SOQL.htm)
